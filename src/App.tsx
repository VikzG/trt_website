...
import { Music, Mic, Radio, Headphones, Menu, X, Play, Volume2, Pause, RotateCcw } from 'lucide-react';
...
const loadAudioDuration = (audioFile: string) => {
  ...
  const handleMetadata = () => {
    ...
    const formatted = tempAudio.duration < 60
      ? `${seconds} sec`
  ...
  return (
    <div className="min-h-screen bg-white cursor-none">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
        scrollY > 50 ? 'bg-black/95 backdrop-blur-sm shadow-lg' : 'bg-transparent'
      }`}>
...
<section id="portfolio" className="py-24 bg-gray-50">
  <div className="max-w-7xl mx-auto px-6">
    ...
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-4">
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-300 ${
                    ...
                    <Volume2 className={`w-8 h-8 transition-colors duration-300 ${
                      playingIndex === index ? 'text-black' : 'text-white'
                    }`} />
                  </div>
                  <span className={`text-sm font-semibold px-3 py-1 rounded-full transition-colors duration-300 ${
                    playingIndex === index ? 'bg-white text-black' : 'text-gray-500 bg-gray-100'
                  }`}>
                    {sound.type}
                  </span>
                </div>
                ...
                <h3 className={`text-xl font-bold mb-2 transition-colors duration-300 ${
                  playingIndex === index ? 'text-white' : 'text-gray-900'
                }`}>
                  {sound.title}
                </h3>
                <p className={`mb-3 transition-colors duration-300 ${
                  playingIndex === index ? 'text-gray-300' : 'text-gray-600'
                }`}>
                  {sound.client}
                </p>
                <div className="flex items-center justify-between">
                  <span className={`text-sm transition-colors duration-300 ${
                    playingIndex === index ? 'text-gray-400' : 'text-gray-500'
                  }`}>
                    {audioDurations[sound.audioFile] || '--:--'}
                  </span>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => handleRestart(index, sound.audioFile)}
                      className="transition-all duration-300"
                    >
                      <RotateCcw className="w-6 h-6 mix-blend-difference" />
                    </button>
                    <button
                      onClick={() => handlePlayPause(index, sound.audioFile)}
                      className="transition-all duration-300"
                    >
                      {playingIndex === index ? (
      ...
      <section id="présentation" className="py-24 bg-black text-white relative overflow-hidden">
        <div className="absolute inset-0 opacity-5">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="text-center mb-16 animate-fade-in-up">
            ...
            <p className="text-xl text-gray-300 max-w-4xl mx-auto leading-relaxed">
              Nous mettons à votre service nos années d'apprentissage au sein de conservatoires, ainsi que notre expérience dans l'industrie musicale, pour vous proposer des créations personnalisées et adaptées à vos besoins.
            </p>
          </div>
          ...
          <div className="max-w-5xl mx-auto">
            <div className="animate-scale-in delay-300 bg-white/5 backdrop-blur-sm rounded-3xl p-12 border border-white/10 hover:bg-white/10 transition-all duration-500">
              <h3 className="text-3xl font-bold mb-8 text-center">Notre mission</h3>
              <p className="text-gray-300 mb-6 leading-relaxed text-lg animate-fade-in delay-400">
                Ces compositions ont pour but de renforcer votre image de marque et d'apporter un coup d'éclat à un moment clé.
              </p>
              <p className="text-gray-300 mb-6 leading-relaxed text-lg animate-fade-in delay-500">
                Nous proposons à la fois des <span className="font-semibold text-white">services durables</span>, pensés pour coller à l'identité de votre marque et réutilisables à l'infini, ainsi que des <span className="font-semibold text-white">services éphémères</span> correspondant à un besoin ou une action précise de votre marque, limitée dans le temps.
              </p>
              <div className="bg-white/5 rounded-2xl p-6 mt-8 border border-white/10 animate-fade-in delay-1000 hover:border-white/20 transition-all duration-300">
                <p className="text-gray-400 leading-relaxed italic text-center">
                  Saisons, campagnes de promotion, événements, lancements de produits... Nous nous adaptons à vos besoins.
                </p>
              </div>
            </div>
          </div>
      ...
      <section id="contact" className="py-24 bg-white text-black relative overflow-hidden">
        <div className="absolute inset-0 opacity-5">
      ...
      <footer className="bg-black text-white py-12">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-4 gap-8 mb-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-gray-400">
                <li><a href="#" className="hover:text-white transition-colors">Composition</a></li>
                ...
                <li><a href="#" className="hover:text-white transition-colors">Identité Sonore</a></li>
              </ul>
            </div>
          ...
          <div className="border-t border-gray-800 pt-8 text-center text-sm text-gray-400">
            <p>&copy; 2025 L.A.S. - Les Artisans Sonore. Tous droits réservés.</p>
          </div>
        </div>
      </footer>
    </div>
